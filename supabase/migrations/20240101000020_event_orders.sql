...
drop trigger if exists event_orders_updated_at on event_orders;
create trigger event_orders_updated_at
  before update on event_orders
  for each row execute function set_updated_at();

drop trigger if exists event_order_items_updated_at on event_order_items;
create trigger event_order_items_updated_at
...
drop trigger if exists event_age_verifications_updated_at on event_age_verifications;
create trigger event_age_verifications_updated_at
