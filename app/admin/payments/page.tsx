...
export default async function AdminPaymentsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-serif text-2xl text-ivory">Paiements</h1>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {payments.map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-b border-white/5 last:border-0 hover:bg-graphite/40">
                    <td className="px-4 py-3">
                      <Link href={`/admin/orders/${p.order_id}`} className="text-champagne hover:underline">
                        {p.order_id.slice(0, 8).toUpperCase()}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-ivory/70">{order?.customer_name ?? "—"}</td>
                    <td className="px-4 py-3 text-champagne">{formatAgorot(p.amount_agorot)}</td>
                    <td className="px-4 py-3 text-ivory/70">{p.method ? paymentMethodLabel(p.method) : "—"}</td>
                    <td className="px-4 py-3">
                      <span className="rounded-full bg-white/5 px-2.5 py-0.5 text-xs text-ivory/80">
                        {STATUS_LABELS[p.status] ?? p.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-xs text-muted-grey">{p.reference ?? "—"}</td>
