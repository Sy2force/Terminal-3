            ...
            {products.map((product) => (
              <tr key={product.id} className="hover:bg-white/[0.02]">
                <td className="px-4 py-3">
                  ...
                  <div className="text-xs text-muted-grey">{product.brand || "—"}</div>
                </td>
...
function InventoryCard({
  ...
  return (
    <div className={`flex flex-col rounded-sm border ${borderColor} bg-graphite/30 p-5`}>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`text-xs ${styles[status] ?? "text-muted-grey"}`}>{labels[status] || status}</span>;
}
