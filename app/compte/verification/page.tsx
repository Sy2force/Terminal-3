...
export default async function CompteVerificationPage() {
  const user = await getCurrentUser();
  if (!user) redirect("/login?redirect=/compte/verification");
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-16 lg:px-8">
      ...
      <div className="mt-8">
        <Link href="/compte" className="text-sm text-champagne hover:underline">
          ← Retour à mon compte
        </Link>
      </div>
    </div>
