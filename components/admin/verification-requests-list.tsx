...
export function VerificationRequestsList({
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 rounded-sm border border-beige-fonce bg-white px-4 py-2.5">
        ...
        <input
          ...
          className="w-full bg-transparent text-sm text-noir-profond outline-none placeholder:text-gris-chaud"
        />
        <button
          type="button"
          onClick={() => refresh()}
          className="text-xs uppercase tracking-wide text-or-principal hover:text-soft-gold"
        >
          Rechercher
        </button>
      </div>
      ...
      <div className="grid gap-4">
        {requests.map((r) => (
          <div
            key={r.userId}
            className="flex flex-col gap-4 rounded-sm border border-beige-fonce bg-creme p-5 sm:flex-row sm:items-start sm:justify-between"
          >
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="flex items-center gap-1.5 rounded-full border border-beige-fonce px-3 py-2 text-xs text-noir-profond/80 hover:border-or-principal/40"
              >
              ...
              <button
                ...
                disabled={isPending}
                className="flex items-center gap-1.5 rounded-full bg-or-principal px-3 py-2 text-xs font-semibold text-noir-profond hover:bg-or-clair disabled:opacity-50"
              >
      ...
      {docUrl && (
        ...
        >
          <div className="max-h-[85vh] max-w-3xl overflow-auto rounded-sm bg-white p-4" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mt-3 text-center text-xs text-gris-chaud">
      ...
      {rejectingUserId && (
        ...
        >
          <div
            className="w-full max-w-md rounded-sm bg-white p-6"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="font-serif text-lg text-noir-profond">Motif du refus</h3>
            <textarea
              autoFocus
              rows={3}
              value={rejectReason}
              ...
              className="mt-3 w-full rounded-sm border border-beige-fonce bg-fond-papier px-4 py-3 text-sm text-noir-profond outline-none focus:border-or-principal"
            />
            <div className="mt-4 flex justify-end gap-2">
              <button
                type="button"
                onClick={() => setRejectingUserId(null)}
                className="rounded-full border border-beige-fonce px-4 py-2 text-xs text-noir-profond/80"
              >
                Annuler
              </button>
              <button
      ...
      {history && (
        ...
        >
          <div className="max-h-[80vh] w-full max-w-lg overflow-auto rounded-sm bg-white p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-serif text-lg text-noir-profond">Historique des décisions</h3>
            {history.length === 0 ? (
              <p className="mt-3 text-sm text-gris-chaud">Aucune décision enregistrée.</p>
            ) : (
              <ul className="mt-3 space-y-3">
                {history.map((h) => (
                  <li key={h.id} className="rounded-sm border border-beige-fonce bg-fond-papier p-3 text-sm">
                    <p className="text-noir-profond">{h.action}</p>
