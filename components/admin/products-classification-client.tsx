...
export function ProductsClassificationClient() {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-champagne/20 bg-fond-papier/40 p-6">
        <h2 className="mb-4 font-serif text-xl text-noir-profond">Testeur de classement</h2>
        <p className="mb-4 text-sm text-noir-profond/70">
          Saisis un nom de produit pour voir la classification automatique proposée. Aucune donnée n’est modifiée.
        </p>
        <div className="relative">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-noir-profond/50" />
          <input
            type="text"
            value={name}
            ...
            className="w-full rounded-full border border-ivory/20 bg-noir-profond py-2.5 pl-10 pr-4 text-sm text-noir-profond placeholder:text-noir-profond/40 focus:border-or-principal focus:outline-none"
          />
        </div>
        {isPending && <p className="mt-2 text-xs text-noir-profond/50">Analyse en cours…</p>}
      ...
      <div className="rounded-xl border border-champagne/20 bg-fond-papier/40 p-6">
        <h2 className="mb-4 font-serif text-xl text-noir-profond">Règles intégrées actives</h2>
        <p className="text-sm text-noir-profond/70">
          {isPending
            ? "Chargement…"
